...
(function (root) {
  ...
  var Game = TTT.Game = function (endgameCallback) {
  ...
  Game.prototype.move = function (pos, callback) {
    ...
    this.placeMark(pos);
    this.switchPlayer();
    if (callback) callback.call(this, pos[0], pos[1]);
  ...
  Game.prototype.over = function() {
    for (var row = 0; row < 3; row++) {
      for (var col = 0; col < 3; col++) {
        if (_.isNull(this.board[row][col])) {
          return false;
        }
      }
    }
